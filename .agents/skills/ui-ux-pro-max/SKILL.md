...
The search script is vendored in this project's `.agents/skills/ui-ux-pro-max/scripts` directory. Run the following commands from the repository root. From another working directory, resolve `scripts/search.py` relative to this `SKILL.md` and use that absolute path:

```bash
python3 .agents/skills/ui-ux-pro-max/scripts/search.py "<query>" --domain <domain>
```

Requires Python 3.x and no external dependencies. On Windows, `py -3` may be used instead of `python3`.
