...
function CampusLeaderLanding({ content, onApply }: CampusLeaderLandingProps) {
  ...
  return (
    <div
      ...
      className="bg-background flex flex-col gap-20 py-16 md:gap-28 md:py-24"
    >
      <section className={homeShellClassName}>
        <div className="grid items-center gap-10 lg:grid-cols-2 lg:gap-16">
          <div className={cn(revealClassName, "flex flex-col gap-4")}>
            <Eyebrow>{landing.what.label}</Eyebrow>
            <h2
              className={cn(
                homeDisplayClassName,
      ...
      <section className={homeShellClassName}>
        ...
        <ol className="border-border flex flex-col divide-y">
          {landing.role.items.map((item, index) => {
            ...
            return (
              <li
                ...
                className={cn(
                  revealClassName,
                  "grid max-w-3xl gap-5 py-8 md:grid-cols-[auto_minmax(0,1fr)] md:items-start md:gap-8 md:py-10",
              ...
              >
                ...
                />
                <div className={cn("flex flex-col gap-2", zigZag && "md:order-1 md:text-right")}>
        ...
        <div className="grid items-start gap-12 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] lg:gap-16">
          <div className={revealClassName}>
            <div className="flex flex-col gap-4">
              <Eyebrow>{landing.benefits.label}</Eyebrow>
              <h2
                className={cn(
                  homeDisplayClassName,
                  "text-3xl normal-case sm:text-4xl md:text-5xl"
          ...
          <figure
            className={cn(
              revealClassName,
              "motion-safe:delay-100 overflow-hidden rounded-[2rem] lg:sticky lg:top-28"
            )}
          >
            <img
              src={media.benefitsSrc}
      ...
      <section className={homeShellClassName}>
        <div
          className={cn(
            homeCardClassName,
            revealClassName,
            "bg-surface-ink text-on-dark flex flex-col gap-6 p-8 md:flex-row md:items-end md:justify-between md:p-10"
          )}
        >
          <div className="max-w-xl">
            ...
            <h2
              className={cn(
                homeDisplayClassName,
                "mt-3 text-3xl text-on-dark normal-case sm:text-4xl"
              )}
            >
              {landing.ctaBand.title}
            </h2>
            <p className="mt-3 text-base leading-relaxed text-on-dark/75">
              {landing.ctaBand.body}
            </p>
            <p className="mt-3 text-sm leading-relaxed text-on-dark/55">
              {content.creditsNote}
            </p>
          </div>
          {content.applicationsOpen ? (
            <Button
              type="button"
              onClick={onApply}
              className={cn(homePillClassName, "h-12 shrink-0 px-8")}
            >
              {landing.ctaBand.applyCta}
            </Button>
          ) : (
            <p className="text-on-dark/70 max-w-xs text-sm leading-relaxed">
              {content.closedBody}
            </p>
          )}
        </div>
